...
\begin{itemize}
    ...
    \item Computer
    \item ROS2-Software (eine Robotik-Software-Plattform, siehe Kapitel 14)
\end{itemize}
...
Solche Projekte prägten viele Grundideen des Systems Engineering: klare Anforderungen, kontrollierte Schnittstellen, Konfigurationsmanagement (der geordnete Umgang mit Änderungen und Versionen), Verifikation (wird weiter unten in diesem Kapitel genauer erklärt), Validierung (die Prüfung, ob das System tatsächlich das richtige Problem löst) und Nachvollziehbarkeit. Der zentrale Gedanke lautet: Man entwickelt nicht nur Bauteile, sondern ein Gesamtsystem mit nachweisbarer Erfüllung seiner Aufgaben.
...
Eine Funktion beschreibt dabei eine Fähigkeit, zum Beispiel \enquote{Hindernisse erkennen}, während eine Komponente das konkrete Bauteil ist, das diese Fähigkeit umsetzt, zum Beispiel ein Lidar-Sensor; dieselbe Funktion kann durch unterschiedliche Komponenten erfüllt werden. Erst danach entscheiden wir, welche Komponenten diese Funktionen erfüllen. Vielleicht braucht der Roboter einen Lidar-Sensor. Vielleicht reicht eine Tiefenkamera nicht aus. Vielleicht ist eine Kombination aus mehreren Sensoren sinnvoll. Die Lösung folgt aus dem Problem, nicht umgekehrt. Abbildung~\ref{fig:kap01-wirkkette} zeigt diese Wirkkette vom Sensor bis zu den Motoren sowie, parallel dazu, den Regelkreis der Energieversorgung.

\begin{figure}[H]
  \centering
  \adjustbox{max width=\linewidth,max totalheight=0.6\textheight}{\input{figures/fig_kap01_wirkkette}}
  \caption{Wirkkette der Hinderniserkennung von der Sensorik bis zu den Motoren, darunter der Regelkreis der Energieversorgung.}
...
Abbildung~\ref{fig:kap01-stakeholder} fasst die zehn wichtigsten Stakeholder des Indoor-Roboters noch einmal zusammen und zeigt jeweils ein typisches Interesse am System.

\begin{figure}[H]
  \centering
  \adjustbox{max width=\linewidth,max totalheight=0.6\textheight}{\input{figures/fig_kap01_stakeholder}}
  \caption{Stakeholder des autonomen Indoor-Roboters mit jeweils einem typischen Interesse am System.}
...
Der Kontext ist wichtig, weil viele Anforderungen an den Grenzen entstehen. Schnittstellen sind häufig Fehlerquellen. Ein Roboter kann intern sauber funktionieren und trotzdem im Betrieb scheitern, wenn die Kommunikation mit der Umgebung unklar ist. Abbildung~\ref{fig:kap01-systemgrenze} veranschaulicht die Systemgrenze und den Kontext mit den wichtigsten Energie-, Daten- und Bewegungsflüssen.

\begin{figure}[H]
  \centering
  \adjustbox{max width=\linewidth,max totalheight=0.6\textheight}{\input{figures/fig_kap01_systemgrenze}}
  \caption{Systemgrenze und Kontext des autonomen Indoor-Roboters mit Energie-, Daten- und Bewegungsflüssen über die Grenze hinweg.}
