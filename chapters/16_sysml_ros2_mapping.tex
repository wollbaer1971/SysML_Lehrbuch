...
Mapping & Bemerkung\\
\midrule
\textbf{SysML:} Part Definition \texttt{ObjectDetector}\newline
\textbf{ROS2:} \texttt{object\_detector\_node} & Führt ML-Inferenz aus\\
\textbf{SysML:} Part Definition \texttt{Camera}\newline
\textbf{ROS2:} \texttt{camera\_node} & Liefert Bilddaten\\
\textbf{SysML:} Part Definition \texttt{NavigationSystem}\newline
\textbf{ROS2:} \texttt{navigation\_node} & Plant und überwacht Bewegung\\
\textbf{SysML:} Part Definition \texttt{BatterySystem}\newline
\textbf{ROS2:} \texttt{battery\_monitor\_node} & Überwacht Akkustatus\\
\textbf{SysML:} Part Definition \texttt{MotionControl}\newline
\textbf{ROS2:} \texttt{motion\_control\_node} & Setzt Geschwindigkeitsbefehle um\\
\textbf{SysML:} Part Definition \texttt{UserInterface}\newline
\textbf{ROS2:} \texttt{user\_interface\_node} & Nimmt Aufträge entgegen\\
\bottomrule
\end{longtable}
\endgroup
...
\begin{figure}[H]
  \centering
  \adjustbox{max width=\linewidth,max totalheight=0.6\textheight}{\input{figures/fig_kap16_mapping_sysml_ros2}}
...
\begin{figure}[H]
  \centering
  \adjustbox{max width=\linewidth,max totalheight=0.6\textheight}{\input{figures/fig_kap16_traceability_batteriemanagement}}
...
Das Mapping verbindet SysML-v2-Modellierung \cite{sysmlv2spec,sysmlv2pilot} mit ROS2-Kommunikation \cite{ros2docs}. In realen Projekten wird diese Verbindung oft
