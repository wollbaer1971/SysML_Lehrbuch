...
Die Dateiendung \texttt{.pt} im Beispiel deutet auf ein PyTorch-Modell hin; bei TensorFlow oder \gls{onnx} Runtime als Framework sähen Modell-Dateiformat und Ladefunktion entsprechend anders aus. Auch der Nachrichtentyp \texttt{DetectionArray} ist hier vereinfacht dargestellt: In der Praxis würde man beispielsweise den Standardtyp \texttt{vision\_msgs/Detection2DArray} verwenden oder einen eigenen Nachrichtentyp definieren.
...
\begin{figure}[H]
  \centering
  \adjustbox{max width=\linewidth,max totalheight=0.6\textheight}{\input{figures/fig_kap18_ml_sicherheitsarchitektur}}
...
\section{Optimierung}
Für reale Roboter können \gls{onnx} Runtime, TensorRT oder OpenVINO sinnvoll sein – plattform- beziehungsweise hardwareoptimierte Laufzeitumgebungen, die trainierte Modelle schneller und ressourcenschonender ausführen. Ein großes PyTorch-Modell direkt auf einem kleinen Rechner kann zu langsam sein. Optimierung betrifft nicht nur Geschwindigkeit, sondern auch Speicherverbrauch, Temperatur und Energie.
